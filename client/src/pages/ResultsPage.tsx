...
function getDisplayVoters(
	isClosed: boolean,
	electionTotalVoters: number | null | undefined,
	liveVoterCount: number
): number {
	return isClosed ? (electionTotalVoters ?? 0) : liveVoterCount
}
...
export function ResultsPage(): JSX.Element {
	...
	const displayVoters = getDisplayVoters(isClosed, electionResults?.totalVoters, voterCount)
		...
		<div className="mx-auto max-w-4xl px-6 py-14">
			...
			{isLive && (
				...
				>
					...
					<h2 className="font-display text-2xl font-semibold text-fg">{t('vote.results.in-progress')}</h2>

					<p className="mt-2 text-sm text-muted">{t('vote.results.in-progress-hint')}</p>

					<p className="mt-5 font-display text-5xl font-bold tabular-nums text-fg">{voterCount}</p>
			...
			<VerificationPanel proposalId={id} options={proposal.options} />
		</div>
	)
}

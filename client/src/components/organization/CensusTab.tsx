...
export function CensusTab({ census, isOrganizer, onManageClick }: Props): JSX.Element {
	const { t } = useTranslation()
	const [query, setQuery] = useState('')

	const trimmed = query.trim().toLowerCase()
	const matchCount = trimmed ? census.filter(a => a.toLowerCase().includes(trimmed)).length : census.length
		...
		<div className="space-y-5">
			<div className="flex flex-wrap items-center gap-3">
				<div className="relative min-w-[220px] flex-1">
					...
					<input
						aria-label="Cerca adreces del cens"
						value={query}

...
const EMPTY_SELECTION = new Set<Address>()
...
export function CensusList({
	...
	async function copyAddress(addr: Address): Promise<void> {
		return navigator.clipboard.writeText(addr).then(() => {
			setCopiedAddr(addr)
	...
	const filtered = trimmed ? census.filter(a => a.toLowerCase().includes(trimmed)) : census
	...
	if (filtered.length === 0)
		return (
			<div className="flex flex-col items-center gap-3 rounded-2xl border border-dashed border-border p-10 text-center text-sm text-muted">
				<SearchX size={20} />
		...
		<div>
			<div className="grid gap-1.5 sm:grid-cols-2 lg:grid-cols-3">
				{visible.map(addr => {
					...
					return selectMode ? (
						<button key={addr} type="button" onClick={() => onToggle?.(addr)} className={className}>
							{content}
						</button>
					) : (

...
function NavChevron({ orientation }: ChevronProps): JSX.Element {
...
function pickerReducer(state: PickerState, action: PickerAction): PickerState {
	switch (action.type) {
		...
		case 'commitHour': {
			const parsed = parseInt(action.raw, 10)
			const clamped = isNaN(parsed) ? parseInt(state.hourStr, 10) : Math.min(23, Math.max(0, parsed))
			return { ...state, hourStr: String(clamped).padStart(2, '0') }
		}
		case 'commitMin': {
			const parsed = parseInt(action.raw, 10)
			const clamped = isNaN(parsed) ? parseInt(state.minStr, 10) : Math.min(59, Math.max(0, parsed))
			return { ...state, minStr: String(clamped).padStart(2, '0') }
		}
		default:
			return state
	}
...
function buttonBorder(hasError: boolean, open: boolean): string {
	if (hasError) return 'border-rose-500'
	if (open) return 'border-primary ring-2 ring-primary/30'
		...
		<div ref={containerRef} className="relative">
			<button
				...
				className={[
					...
					buttonBorder(Boolean(hasError), open)
				].join(' ')}
			>
				<CalendarDays size={14} className={displayValue ? 'text-primary' : 'text-muted'} />

				<span className={displayValue ? 'text-fg' : 'text-muted'}>{displayValue || placeholder}</span>
			</button>
					...
					>
						<DayPicker
							...
							classNames={{
								...
								nav: 'flex gap-1',
								button_previous:
									'flex size-7 items-center justify-center rounded-lg text-muted transition hover:bg-elevated hover:text-fg',
								button_next:
									'flex size-7 items-center justify-center rounded-lg text-muted transition hover:bg-elevated hover:text-fg',
								month_grid: 'w-full',
								weekdays: '',
								weekday:
									'py-1 text-center text-[10px] font-semibold uppercase tracking-wide text-muted',
								weeks: '',
								week: '',
								day: 'p-0.5 text-center',
								day_button:
									'mx-auto flex size-8 items-center justify-center rounded-full text-xs font-medium transition text-fg hover:bg-elevated focus:outline-none',
								selected:
									'[&>button]:bg-gradient-to-br [&>button]:from-primary [&>button]:to-accent [&>button]:text-primary-fg [&>button]:shadow-glow [&>button]:hover:brightness-110',
						...
						<div className="border-t border-border bg-elevated/30 p-4">
							<div className="flex items-center justify-center gap-3">
								<input
									...
									max={23}
									value={hourStr}
									onChange={event => dispatch({ type: 'setHourStr', value: event.target.value })}
									onBlur={event => dispatch({ type: 'commitHour', raw: event.target.value })}
								...
								<input
									...
									max={59}
									value={minStr}
									onChange={event => dispatch({ type: 'setMinStr', value: event.target.value })}
									onBlur={event => dispatch({ type: 'commitMin', raw: event.target.value })}
									onFocus={event => event.target.select()}
									className={timeInput}
								/>
							</div>

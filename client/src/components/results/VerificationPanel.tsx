...
export function VerificationPanel({ proposalId, options }: VerificationPanelProps): JSX.Element {
	...
	return (
		<div className="mt-14 rounded-3xl border border-border bg-elevated p-6">
			...
			<div className="flex flex-col gap-3 sm:flex-row">
				<input
					aria-label="Verifica el teu vot"
					value={input}
					onChange={event => {
							...
							<div className="mt-3">
								...
								<ol className="space-y-1">
									{ballot.map((optionId, i) => {
										const opt = options.find(option => option.id === optionId)
			...
			{hasResult && !isError && !voted && (
				...
				>
					...
					<div className="text-sm text-amber-400">{t('results.verify.not-voted')}</div>
				</m.div>
			)}
			...
			{isError && (
				...
				>
					...
					<div className="text-sm text-rose-400">{t('errors.network')}</div>
				</m.div>
			)}
		</div>
	)

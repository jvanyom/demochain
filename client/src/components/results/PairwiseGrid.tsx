...
export function PairwiseGrid({ options, pairwiseMatrix }: PairwiseGridProps): JSX.Element {
	...
	return (
		...
		>
			<div className="mb-5">
				<h3 className="text-sm font-semibold text-fg">{t('results.pairwise-section')}</h3>

				<p className="mt-1 text-xs leading-relaxed text-muted/80">{t('results.pairwise-explanation')}</p>
			</div>
			...
			<div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
				{options.flatMap((optA, ai) =>
					options.slice(ai + 1).map(optB => {
						...
						const resultA = toResult(votesA > votesB, isTied)
						const resultB = toResult(votesB > votesA, isTied)
						...
						return (
							...
							>
								<div className="flex items-center gap-3 px-4 pb-3 pt-4">
									<span className={`flex-1 truncate text-sm font-semibold ${labelClass[resultA]}`}>
										{optA.title}
									</span>

									<span className="shrink-0 rounded-full border border-border px-2 py-0.5 text-xs font-bold text-muted/40">
										vs
									</span>

									<span
										className={`flex-1 truncate text-right text-sm font-semibold ${labelClass[resultB]}`}
									>
										{optB.title}
									</span>
								</div>
								...
								<div className="mx-4 flex h-14 gap-1 overflow-hidden rounded-xl">
									<div
										style={{ flex: votesA + 1 }}
										className={`flex items-center justify-center font-display text-2xl font-bold tabular-nums ${barClass[resultA]}`}
									>
										{votesA}
									</div>
									...
									<div
										style={{ flex: votesB + 1 }}
										className={`flex items-center justify-center font-display text-2xl font-bold tabular-nums ${barClass[resultB]}`}
									>
										{votesB}
									</div>
								</div>

...
export function ApprovalBar({ yes, total, compact = false }: Props): JSX.Element {
		...
		<div className="w-full">
			{!compact && (
				<div className="mb-2 flex items-center justify-between text-xs">
					<span className="font-medium text-muted">{t('proposal.approval.label')}</span>

					<span className="tabular-nums text-muted">{t('common.completeness', { current: yes, total })}</span>
				</div>
			)}
			...
			{!compact && (
				<div className="mt-1.5 flex items-center justify-between text-xs">
					<span className="text-muted">{t('proposal.approval.threshold')}</span>

...
function formatDatetime(str: string): string {
	...
	return isNaN(date.getTime()) ? str : date.toLocaleString(undefined, { dateStyle: 'medium', timeStyle: 'short' })
}
...
interface OrganizationEligibilityGateProps {
	isConnected: boolean
	isLoading: boolean
	elegibleOrganizationsCount: number
	children: ReactNode
}

function OrganizationEligibilityGate({
	isConnected,
	isLoading,
	elegibleOrganizationsCount,
	children
}: OrganizationEligibilityGateProps): JSX.Element {
	...
	if (isLoading && elegibleOrganizationsCount === 0)
		return <div className="h-14 animate-pulse rounded-xl bg-elevated" />

	if (elegibleOrganizationsCount === 0)
		return (
...
export function OrgStep({
	...
	selectedOrgId,
	onSelectOrg
}: OrganizationStepProps): JSX.Element {
	const { t } = useTranslation()
	...
	return (
		<div className="space-y-4">
			...
			<OrganizationEligibilityGate
				isConnected={isConnected}
				isLoading={isLoading}
				elegibleOrganizationsCount={eligibleOrganizations.length}
			>
					...
					{eligibleOrganizations.map(org => (
						...
						>
							...
							<span>
								...
								<span className="mt-0.5 line-clamp-1 block text-xs text-muted">{org.description}</span>
							</span>
						</label>
					))}
				</div>
	...
	return (
		<>
			<Field label={t('proposal.new.fields.title')}>
				...
				{fieldError('title') && (
					<span className="mt-1 block text-xs text-rose-500">{t(`errors.${fieldError('title')}`)}</span>
				)}
			</Field>
			...
			<Field label={t('proposal.new.fields.description')}>
				...
				{fieldError('description') && (
					<span className="mt-1 block text-xs text-rose-500">{t(`errors.${fieldError('description')}`)}</span>
				)}
			</Field>
		</>
	)
			...
			<div className="grid gap-6 sm:grid-cols-2">
				<Field label={t('proposal.new.fields.start')}>
					<DateTimePicker
						...
						hasError={Boolean(fieldError('startDate'))}
					/>
				...
				<Field label={t('proposal.new.fields.end')}>
					<DateTimePicker
						...
						hasError={Boolean(fieldError('endDate'))}
					/>

					{fieldError('endDate') && (
						<span className="mt-1 block text-xs text-rose-500">{t(`errors.${fieldError('endDate')}`)}</span>
					)}
				</Field>
			</div>

			{showDevWarning && <p className="text-xs text-muted">{t('errors.proposal.starting-too-soon')}</p>}
		</div>
	)
}
...
export function OptionsStep({
	...
	return (
		<div className="space-y-3">
			...
			{fieldError('options') && <p className="text-xs text-rose-500">{t(`errors.${fieldError('options')}`)}</p>}
...
export function ReviewStep({ orgName, title, description, start, end, options }: ReviewStepProps): JSX.Element {
	const { t } = useTranslation()

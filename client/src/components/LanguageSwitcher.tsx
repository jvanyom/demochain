					...
					<div className="absolute right-0 top-12 z-40 min-w-[140px] overflow-hidden rounded-xl border border-border bg-elevated shadow-lg">
						{LANGUAGES.map(language => (
							<button
								...
								type="button"
								onClick={() => {
									void i18n.changeLanguage(language.code).then(() => setOpen(false))
								}}
								className={`flex w-full items-center justify-between px-4 py-2 text-sm transition hover:bg-surface ${
									current === language.code ? 'text-primary' : 'text-fg'
								}`}
							>
								<span>{t(`language.${language.code}`)}</span>

								<span className="text-xs font-semibold uppercase text-muted">{language.label}</span>
							</button>
						))}
					</div>
				</>

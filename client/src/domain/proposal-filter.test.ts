...
import type { ProposalStateKind } from './proposal-state'

import { describe, it, expect } from 'bun:test'

import { proposalMatchesFilter } from './proposal-filter'

function mockProposal(stateKind: ProposalStateKind): Proposal {
	const tally = { votesFor: 0, totalVotes: 0 }

	const state = {
		PendingApproval: { kind: 'PendingApproval' as const, approvalTally: tally, memberCount: 0 },
		Rejected: { kind: 'Rejected' as const, approvalTally: tally, memberCount: 0 },
		PendingStart: { kind: 'PendingStart' as const },
		Open: { kind: 'Open' as const },
		Closed: { kind: 'Closed' as const }
	}

	// oxlint-disable-next-line no-unsafe-type-assertion
	return { state: state[stateKind] } as Proposal
}

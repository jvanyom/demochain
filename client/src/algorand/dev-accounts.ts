...
export function getDevAddresses(): Address[] {
	return (rawDevAccounts as DevAccountEntry[]).flatMap(entry =>
		entry?.address && isAddress(entry.address) ? [entry.address] : []
	)
}
...
function resolveDevAccounts(entries: DevAccountEntry[]): ResolvedDevAccount[] {
	...
	for (const entry of entries) {
		...
		try {
			...
			const derived = { addr: account.addr.toString(), sk: account.sk }
...
async function fundDevAccounts(
	...
	const balanceResults = await Promise.all(
		resolved.map(async account => {
			try {
				...
				return { address: account.address, amount: info.amount }
			} catch (err) {
	...
	const underfunded = balanceResults.flatMap(result =>
		result !== null && result.amount < FUND_THRESHOLD_MICRO_ALGOS ? [result.address] : []
	)
	...
	console.info(`[devAccounts] auto-funded ${fundResults.filter(Boolean).length}/${underfunded.length} dev accounts`)
}
...
export async function bootstrapDevAccountsToKmd(): Promise<void> {
	...
	try {
		...
		const wallet = wallets.find((_wallet: { name: string; id: string }) => _wallet.name === KMD_WALLET_NAME)

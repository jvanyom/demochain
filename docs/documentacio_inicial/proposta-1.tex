...
\begin{titlepage}
    ...
    \vspace{0.3cm}
    \begin{tabular}{ll}
        Marc Link     & Coordinador del projecte / \textit{front-end} \\
...
\begin{longtable}{p{4cm}p{4cm}p{2.5cm}p{3.5cm}}
    ...
    \addlinespace
    Interfície web & \textit{front-end} React per a l'emissió de vot i visualització & Votants, Organitzadors & Accessibilitat i usabilitat \\
    \addlinespace
...
\subsection{\textit{front-end} web}
...
\begin{itemize}
    ...
    \item Generació i registre de tokens de vot d'un sol ús (\textit{hash} + nullificador)
    \item Verificació de pertinença al cens electoral
    \item API REST per comunicar \textit{front-end} i blockchain
\end{itemize}
...
Al nostre prototip, la separació identitat-vot s'implementa via la tècnica del \textit{commit-reveal} descrita a la secció anterior. Quan el votant fa clic a "votar", el \textit{front-end} envia la transacció amb el \textit{nullifier} directament a la blockchain; el backend només ha participat en la fase de registre. En el moment de l'emissió del vot, el backend \textbf{no} coneix ni el \textit{nullifier} ni l'opció de vot.
...
\begin{itemize}
    ...
    \item \textbf{bcrypt}: \textit{\textit{hash}ing} segur de contrasenyes amb \textit{salt} automàtic.
...
\begin{longtable}{p{2.5cm}p{3cm}p{3cm}p{3.5cm}p{2cm}}
    ...
    \midrule
    Marc Link & Coordinador + \textit{front-end} & React, gestió àgil, comunicació & Estructura del document, planificació, seccions de motivació i presentació & 30\% \\
    \addlinespace
...
\begin{itemize}
    ...
    \item \textbf{Risc tècnic:} la integració \textit{front-end}-blockchain via \texttt{ethers.js} és identificada com a tasca de risc alt (dependències externes, canvis d'API). El PERT pessimista per a aquesta tasca és el doble del cas optimista.
\end{itemize}
...
\begin{longtable}{p{4.5cm}rrrr}
    ...
    \addlinespace
    \multicolumn{5}{l}{\textbf{Fase 4 - \textit{front-end} i UX}} \\
    Implementació de la interfície de votació (React) & 6 & 10 & 18 & 10.7 \\
    Integració \textit{front-end}-blockchain (ethers.js) & 4 & 8  & 16 & 8.7 \\
...
\begin{enumerate}
    \item El \textbf{smart contract} (Fase 2) és un prerequisit de la integració backend-blockchain i de la integració \textit{front-end}-blockchain.
    \item L'\textbf{API d'autenticació} (Fase 3) és un prerequisit del flux complet de votació.
    \item La integració \textbf{\textit{front-end}-blockchain} (Fase 4) és el darrer component del camí crític.
\end{enumerate}

El camí crític és: \textit{Fase 1 → Smart Contract → Integració → \textit{front-end}}. Qualsevol retard en el \textit{smart contract} afecta directament la data de lliurament.
...
\begin{itemize}
    ...
    \item Durant la \textbf{Fase 3} (backend), el \textit{front-end} pot implementar les pantalles estàtiques i la lògica de navegació sense connexió real a la blockchain (\textit{mocking}).
...
10--11 & Impl. \textit{front-end} React & Suport \textit{front-end} & Endpoints pendents & Proves end-to-end \\
...
El principal risc de col·l d'ampolla és el \textit{smart contract}: si la seva implementació s'allarga més de la Fase 2 prevista, el backend i el \textit{front-end} no podran integrar-se amb els paràmetres definitius. La mitigació és doble: (1) definir una interfície mínima viable del contracte a la setmana 3, i (2) usar \textit{mocking} al backend i \textit{front-end} fins que el contracte real estigui llest.
...
\begin{longtable}{p{3.5cm}p{2cm}p{2cm}p{4.5cm}}
    ...
    \addlinespace
    Fallades d'integració \textit{front-end}-blockchain & Mitjana & Alt & Ús de \textit{mocking} fins a disposar del contracte final; proves d'integració automatitzades \\
    \addlinespace
...
\textbf{Commit-reveal:} esquema de dos passos on primer s'envia un \textit{hash} del vot (\textit{commitment}) i posteriorment el valor original (\textit{reveal}), evitant que el contingut sigui conegut abans de tancar la votació.
...
\subsubsection*{\textit{front-end} web}

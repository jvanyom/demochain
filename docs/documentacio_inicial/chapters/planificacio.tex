...
\hline
\textbf{Lliurables i Tasques} & \textbf{Coordinador} & \textbf{Arq. i Contractes} & \textbf{\textit{front-end} UX} & \textbf{QA i Seguretat} \\
...
% En un enfocament seqüencial tradicional, cada fase s'inicia únicament després de finalitzar l'anterior (per exemple: disseny → desenvolupament backend → desenvolupament \textit{front-end} → integració → proves). Aquest model incrementa la durada total del projecte i concentra els riscos en les fases finals.
...
% El solapament permet reduir el temps total del projecte. Per exemple, el desenvolupament del \textit{front-end} pot iniciar-se un cop definides les interfícies de comunicació amb el backend, sense esperar a la finalització completa d'aquest.
...
% \item el \textit{front-end} comença quan les interfícies API estan definides,
...
% Aquest solapament permet treballar en paral·lel, iniciar el \textit{front-end} quan les interfícies ja estan definides, validar components abans de la integració final i reduir riscos tècnics.
...
% \hline
% 7 & \textit{front-end} votació & UI votació i connexió wallet & \textit{front-end} MVP vot & \textit{front-end} \\
% \hline
% 8 & \textit{front-end} resultats & Resultats públics i millores UX & \textit{front-end} v1 & \textit{front-end} + QA \\
% \hline
...
% \hline
% \textbf{Entregable} & \textbf{PM} & \textbf{Backend} & \textbf{\textit{front-end}} & \textbf{QA} \\
% \hline
...
% \item El \textbf{desenvolupador \textit{front-end}} és responsable de la interfície web i de la interacció amb la blockchain, assegurant una experiència d'usuari clara i funcional.
...
% \node[box, below=1.8cm of backend] (\textit{front-end}) {\textit{front-end}};
% \node[box, below=1.8cm of \textit{front-end}] (qa) {QA};
...
% \draw[arrow] (\textit{front-end}) -- (integration);
...
% \item la integració amb el \textit{front-end}, que permet la interacció de l'usuari amb el sistema,

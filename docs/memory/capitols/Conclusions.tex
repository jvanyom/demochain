...
\begin{itemize}
    ...
    (creació, cens d'adreces autoritzades amb càrrega massiva que es 
    gestiona realitzant grup de transaccions o cada transacció carrega
    set adreces ), propostes amb fase d'aprovació per
    ...
    en Solidity, desplegable a la \emph{testnet} Sepolia, que
    ...
    \cite{schulze2011newmethod} computat localment a partir de les
    papeletes individuals --el contracte d'Algorand no emmagatzema cap
    resultat precalculat, només els vots, fet que reforça l'argument
    de verificabilitat pública--, i panell de verificació pública que
    permet consultar el vot emès per qualsevol adreça.
...
verificabilitat pública sense exigir una participació activa a
...
Aquesta línia està documentada amb detall a les èpiques E3, E4 i E5
del repositori \cite{demochainrepo} i requereix una dedicació
...
vots i coerció. Aquest mecanisme està documentat a la \ac{US} E3-US4
del repositori \cite{demochainrepo} i està implementat al
\emph{state-of-the-art} per sistemes com DAVINCI; és imprescindible
...
\begin{enumerate}
    ...
    \item \textbf{Distribució del client web als votants.} Cada votant es descarrega el codi obert
    del client des del repositori públic \cite{demochainrepo} i
    l'executa localment a la seva pròpia màquina. Aquesta decisió
    reforça la verificabilitat --qualsevol pot inspeccionar el codi
    que executa-- i elimina un punt central de compromís en què un
    tercer podria servir una versió alterada del client a un subconjunt
    de votants. Per a producció caldria, doncs, publicar versions
    \emph{taggejades} i reproduïbles del client, amb instruccions
    clares per a la construcció local (\texttt{bun install} +
    \texttt{bun run dev} o \texttt{bun run build}) i, opcionalment,
    \emph{builds} verificables firmats criptogràficament pels
    mantenidors.
...
del que sembla quan es resten les setmanes amb càrrega laboral o
el trball final de grau. Una planificació amb \emph{sprints} de

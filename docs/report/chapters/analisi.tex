...
\begin{itemize}
    ...
    \item \textbf{Proves de Coneixement Zero (ZK-SNARK)}~\cite{goldwasser1989knowledge,groth2016size}: Són proves criptogràfiques que permeten a un elector provar
    ...
    Mitjançant un protocol de generació de claus distribuïdes (DKG)~\cite{gennaro1999dkg}, la clau privada de desxifratge es divideix en
...
cosa condiciona la manera en què el client ha d'enviar les transaccions a la xarxa.
Finalment, el cost econòmic de fer crides a Ethereum L1 ens fa elegir emprar la xarxa de proves Sepolia, 
i el protocol només envia un únic \textit{hash} de 32 bytes (\texttt{bytes32}) per cada elecció
...
En l'àmbit legal, el disseny s'enfronta directament a la normativa europea de protecció de dades (RGPD)~\cite{eu2016gdpr}.
...
la xarxa.
Es pensava que mitjançant el mecanisme de VRF (Funció Aleatòria Verificable)~\cite{micali1999vrf} d'Algorand es podria obligar cada votant a
...
Dins de la teoria de l'elecció social, el \textbf{Teorema de la Impossibilitat d'Arrow}~\cite{arrow1963socialchoice} demostra matemàticament que cap
...
dades, s'ha adoptat una aproximació de \textbf{Domain-Driven Design (DDD)}~\cite{evans2003ddd} definint un llenguatge comú per a tot el
projecte.
